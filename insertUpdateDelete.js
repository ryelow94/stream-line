...
function handleSubmit(event) {
  ...
  if (!date_regex.test(event.target[6].value)) {
    alert(
...
//   edition.textContent = event.target[6].value;
  dueDate.textContent = event.target[6].value;
  priority.textContent = event.target[7].value;
  escalate.textContent = event.target[8].value;
  approved.textContent = event.target[9].value;
  explanation.textContent = "Please test this in a database backup and, if successful, please execute against the requested environment." + " " + event.target[10].value;
